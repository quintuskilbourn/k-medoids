...
class Kmed:
    ...
    def db_v2(self,S,lvln,lvlBound): #can be optimised so that TD can be run when before all nodes are formed
        Q = self.dbQ
        rem = S
        starttime = timeit.default_timer()
        ...
        for lvl in range(max(0,S[0]-1),min(S[0]+2,m)): #includes m
            bound += len(lvln[lvl])
        degDict = {node:self.G.degree[node] for lvl in S for node in lvln[lvl]}
        ...
        while i < len(S):
            ...
            if reps:
                ...
                reps = 0
                if superQ:
                    superQ = [supr+list(c) for c in combs for supr in superQ]
                else:
                    superQ = list(map(list,combs))
                i+=1
                continue
            if superQ:
                superQ = [supr+[n] for supr in superQ for n in lvln[S[i]]]
            else:
                superQ = [[n] for n in lvln[S[i]]]
            i+=1
        cnt = 0
        ...
        while 1:
            if self.lowest_centrality<=self.dbQ.front()[0]:
                if self.lowest_centrality<=self.lQ.front()[0]:
                    return {'Opt_val':self.lowest_centrality,
                            'Opt_set':self.lowest_supernode,
                            'FP2_val':self.approx[0],
                            'FP2_set':self.approx[1],
                            'FP2_2_val':self.approx_2[0],
                            'FP2_2_set':self.approx_2[1],
